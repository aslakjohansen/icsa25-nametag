...
\newcommand{\OptionBanner}[1]{
  \begin{tikzpicture}[remember picture,overlay]
    \draw[fill=#1]
         ([xshift= \margin,yshift=28mm] current page.south west)
      [rounded corners=6mm/1.4142135624] % \sqrt{2} = 1.4142135624
      -- ([xshift= \margin,yshift=\margin] current page.south west)
      -- ([xshift=-\margin,yshift=\margin] current page.south east)
      [sharp corners]
      -- ([xshift=-\margin,yshift=28mm] current page.south east)
      -- cycle
    ;
  \end{tikzpicture}
}
...
%      {\textsl{sponsor}};
    ...
    \node[anchor=center,text=black]
      ...
      {\includegraphics[scale=0.03]{figs/sdu.pdf}};
